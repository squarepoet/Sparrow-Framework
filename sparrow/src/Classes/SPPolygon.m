...
@interface SPEllipsePolygon : SPImmutablePolygon
...
{
    return [[[SPEllipsePolygon alloc] initWithX:x y:y radiusX:radius radiusY:radius numSides:-1] autorelease];
}

+ (instancetype)ellipseWithX:(float)x y:(float)y radiusX:(float)radiusX radiusY:(float)radiusY
{
    return [[[SPEllipsePolygon alloc] initWithX:x y:y radiusX:radiusX radiusY:radiusY numSides:-1] autorelease];
}
...
#pragma mark - SPEllipsePolygon

@implementation SPEllipsePolygon
{
    float _x;
    float _y;
    float _radiusX;
