...
{
    __SP_GENERICS(NSMutableDictionary,NSString*,__SP_GENERICS(NSArray,SPEventListener*)*) *_eventListeners;
}
...
{
    __SP_GENERICS(NSArray,SPEventListener*) *listeners = _eventListeners[event.type];
...
- (void)addEventListener:(SPEventListener *)listener forType:(NSString *)eventType
{
    ...
    __SP_GENERICS(NSArray,SPEventListener*) *listeners = _eventListeners[eventType];
    if (!listeners)
    {
                        ...
                        andSelector:(SEL)selector orBlock:(SPEventBlock)block
{
    ...
    if (listeners)
    {
        __SP_GENERICS(NSMutableArray,SPEventListener*) *remainingListeners = [[NSMutableArray alloc] init];
        for (SPEventListener *listener in listeners)
        {
